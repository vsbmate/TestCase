...
@Controller
public class AllAbonentsGetter {

    private final AbonentRepository abonentRepository;

    public AllAbonentsGetter(AbonentRepository abonentRepository) {
        this.abonentRepository = abonentRepository;
    }

    @GetMapping("/getallabonents")
    @ResponseBody
    public String getAllAbonents(@RequestParam(defaultValue = "0") int page,
        ...
        sb.append("<a href=\"/getallabonentssortedbyaccount" + "\">Отсортировать по лицевому счету(account) \n </a>");
        ...
        if (abonentsPage.hasNext()) {
            sb.append("<a href=\"/getallabonents?page=" + (page+1) + "&size=" + size + "\">Следующий </a>");
        }
        if (abonentsPage.hasPrevious()) {
            sb.append("<a href=\"/getallabonents?page=" + (page-1) + "&size=" + size + "\">Предыдущий </a>");
        }
        return sb.toString();
